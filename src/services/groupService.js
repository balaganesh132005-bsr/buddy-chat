...
// requesterId = who's performing the action. Only the group admin can remove
// *other* people; anyone can remove themselves (leaveGroup uses this too).
...
// Update group (name, photoURL, etc.)
...
// Transfer admin/ownership to another member of the group
export const transferGroupOwnership = async (groupId, currentOwnerId, newOwnerId) => {
  try {
    ...
    if (!groupData.members.includes(newOwnerId)) {
      throw new Error("New admin must already be a member of the group");
    }
...
// Delete an entire group (only the owner/admin can do this —
// enforce the same check in your Firestore security rules, not just here)
